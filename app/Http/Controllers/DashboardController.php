...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $upcoming_elections_all = DB::table('elections')->where('status', '=', '')->get();
        $upcoming_elections = DB::table('elections')->where('status', '=', '')->paginate(3);
        $opened_elections_all = DB::table('elections')->where('status', '=', 'open')->get();
        $opened_elections = DB::table('elections')->where('status', '=', 'open')->paginate(3);
        $closed_elections = DB::table('elections')->where('status', '=', 'closed')->get();
        ...
        return view('user.dashboard',
            [
                ...
                'admins' => $admins,
                'elections' => $elections,
                'upcoming_elections_all' => $upcoming_elections,
                'upcoming_elections' => $upcoming_elections,
                'opened_elections_all' => $opened_elections,
