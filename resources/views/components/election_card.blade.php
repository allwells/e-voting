...
<div
    ...
    <div class="z-20 flex items-center justify-end w-full px-2 pt-2">
        @if ((auth() && auth()->user()->privilege == 'superuser') || (auth() && auth()->user()->privilege == 'admin'))
            ...
            <!-- Dropdown menu -->
            <div id="dropdownLeft-{{ $election->id }}" aria-labelledby="dropdownLeftButton"
                ...
                <ul class="text-sm text-left text-neutral-700 dark:text-neutral-200">
                    ...
                    <li class="p-1.5 @if (!($election->status === 'closed')) border-t dark:border-neutral-600 @endif">
    ...
    <div
        ...
        <p
            class="text-sm font-light text-left sm:text-lg text-neutral-700 dark:text-neutral-400 line-clamp-2 lg:line-clamp-1">
            {{ $election->description }}
        </p>
    </div>
    ...
    <div class="flex flex-col items-start justify-between w-full gap-3 p-4 lg:p-6 md:items-end md:flex-row">
        ...
        <div
            class="flex flex-col items-end justify-end w-full gap-2 lg:flex-col lg:w-6/12 md:w-fit md:flex-row md:gap-3">
            ...
            @if ((auth() && auth()->user()->privilege === 'superuser') || (auth() && auth()->user()->privilege === 'admin'))
                @if ($today->lt($election->start_date) && $today->lt($election->end_date) && $election->status === '')
